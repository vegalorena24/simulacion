...
! Read coordinates of the trajectory (restart xyz file)
  open(unit=127,file='restart.rst',status='unknown',action='read')
    do i=1,snaps*part
      read(127,*), input(i,1), input(i,2), input(i,3)
    end do
  close(127)
...
call desplace(input,snaps,dimen,part,desplacement)


! Write output file
  open(unit=138,file='desplazamientos.dat',status='unknown',action='write')
    do i=1,snaps
      write(138,*), desplacement(i,:)
    end do
  close(138)
